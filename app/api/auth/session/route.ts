...
export async function GET(request: NextRequest) {
  try {
    ...
    if (identity) {
      ...
      const response = json({
        authenticated: true,
        user: { name: user.name, email: user.email, role: user.role, tier: user.tier, viewer: user.viewer, district: user.district, school: user.school },
